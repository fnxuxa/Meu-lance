...
- Proteção contra negociação fora da plataforma (25/09/2026): o chat do pedido só grava pela RPC `send_order_message` (INSERT direto revogado; o trigger `guard_order_message` é a segunda barreira). Antes do pagamento (`order_contact_released` = lista branca: paid, awaiting_shipment, shipped, delivered, completed, disputed, resolved_*), `contact_pattern` (nível 1: telefone de 8–13 dígitos com separadores/letras no lugar de dígito, e-mail, link, apps de contato) bloqueia e registra em `chat_flagged_attempts` (só a equipe lê, via `staff_list_chat_flags`, painel admin). Não há punição automática. Palavras comuns (celular, número, contato) NÃO bloqueiam sozinhas. Nome completo e endereço da contraparte só via `get_order_counterparty` após o pagamento; nunca telefone, e-mail ou CPF. Apelido público é "Nome S." (`short_display_name`). Validar prazo de retenção de `chat_flagged_attempts` (dado pessoal) com advogado.
